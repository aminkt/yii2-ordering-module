...
//                ]
            ...
            [
                ...
                /** @var \ordering\interfaces\OrderInterface $model */
                    ...
                    //$html .= Html::a('<i class="fa fa-eye"></i>', ['view', 'id'=>$model->id], ['class'=>'btn btn-icon-only green']);
                    $html .= Html::a('<i class="fa fa-eye"></i>', '#', [
                        ...
                        'data'=>[
                            ...
                            'target'=>'#show-order',
                            'toggle'=>'modal',
                            'url'=>\yii\helpers\Url::to(['view', 'id'=>$model->id, 'page'=>$this->context->action->id]),
                            'title'=>'مشخصات سفارش شماره '.$model->getTrackingCode(),
                            'loader-container'=>'.order-content',
                        ]
                    ]);
                    $html .= Html::a('<i class="fa fa-check"></i>',
                        [
                            'change-status',
                            'id'=>$model->getId(),
                            'page'=>$this->context->action->id,
                            'do'=>$model::STATUS_READY_TO_SEND
                        ], ['class'=>'btn btn-icon-only green tooltips', 'data'=>[
                            'container'=>'body',
                            'placement'=>'top',
                            'original-title'=>'انتقال به آماده ارسال'
                        ]]);
                    $html .= Html::a('<i class="fa fa-refresh"></i>', [
                        'change-status',
                        'id'=>$model->getId(),
                        'page'=>$this->context->action->id,
                        'do'=>$model::STATUS_WAITING_FOR_CONFIRM
                    ], ['class'=>'btn btn-icon-only yellow tooltips', 'data'=>[
                        'container'=>'body',
                        'placement'=>'top',
                        'original-title'=>'برگشت به در انتظار تائید',
                        'confirm'=>'از برگرداندن این مورد اطمینان دارید؟',
                        'method'=>'post'
                    ]]);
                    $html .= '</div> </div>';
                    return $html;
